...
--- @return boolean
function M.IsAwsCliAvailable()
    return packages.IsToolInstalled("aws")
end

--- Check if the SSM plugin is installed.
--- @return boolean
function M.IsSsmPluginAvailable()
    return packages.IsToolInstalled("session-manager-plugin")
end

--- Ensure required tools are available.
--- @return boolean
function M.EnsureAvailable()
    if not M.IsAwsCliAvailable() then
        print("AWS CLI is required for secure tunnel.Connections.")
        if not packages.PromptInstall("awscli") then
            return false
        end
    end
    if not M.IsSsmPluginAvailable() then
...
--- @return string
function M.BuildSsmSessionCommand(instance_id, profile, region)
...
--- @return string
function M.BuildSsmPortForwardCommand(instance_id, remote_port, local_port, profile, region)
    local parts = { "aws", "ssm", "start-session" }
    table.insert(parts, "--target " .. instance_id)
...
--- @param region string|nil
--- @return string
function M.BuildEc2IcCommand(instance_id, user, profile, region)
...
--- @param region string|nil
--- @return string
function M.BuildExecCommand(instance_id, remote_cmd, profile, region)
...
function M.Connect(conn_data)
    if not M.EnsureAvailable() then return end
    ...
    if tunnel_type == M.TYPE_SSM_SESSION then
        cmd = M.BuildSsmSessionCommand(instance_id, profile, region)
    elseif tunnel_type == M.TYPE_SSM_PORT_FWD then
        ...
        cmd = M.BuildSsmPortForwardCommand(instance_id, remote_port, local_port, profile, region)
    elseif tunnel_type == M.TYPE_EC2_IC then
        cmd = M.BuildEc2IcCommand(instance_id, user, profile, region)
    else
